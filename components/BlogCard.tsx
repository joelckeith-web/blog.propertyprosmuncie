...
export default function BlogCard({ post }: { post: BlogPost }) {
  ...
  const categoryColors: Record<string, string> = {
    ...
    fencing: "bg-yellow-100 text-yellow-800",
    remodeling: "bg-purple-100 text-purple-700",
    construction: "bg-orange-100 text-orange-700",
    general: "bg-gray-100 text-gray-700",
    decks: "bg-teal-100 text-teal-700",
  };
  ...
  return (
    <article className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow group">
      ...
      <div className="p-6">
        ...
        {/* Title */}
        <h2 className="text-lg font-bold text-brand-dark mb-2 group-hover:text-brand-orange transition-colors line-clamp-2">
          <Link href={`/${frontmatter.slug}`}>{frontmatter.title}</Link>
        </h2>

        {/* Description */}
        <p className="text-sm text-brand-dark-secondary mb-4 line-clamp-3">
